...
info_df, mag_df = binocs.readdata(options)
print(mag_df)
...
results = binocs.sedfit(singles, binary, mag_df, options)
...
with open(options['data'] + "--binary.txt", "w") as out:
    for s in range(len(mag_df)):
        # Print out star to file
        outstr = "%16s %9.5f %9.5f " % (info_df.loc[s, '2MASS ID'], info_df.loc[s, 'RA'], info_df.loc[s, 'DEC'])
        for i in range(17):
            outstr += "%6.3f " % mag_df.iloc[s, i*2]
        outstr += "   %2d %2d %6.3f %6.4f %6.3f %6.4f %5.2f   %6.3f %6.4f %5.2f" % (summary[s, 8], info_df.loc[s, 'RV'], summary[s, 0], summary[s, 1], summary[s, 2], summary[s, 3], summary[s, 4], summary[s, 5], summary[s, 6], summary[s, 7])
...
synth = binocs.makesynth(mag_df, binary, options)
...
with open(options['data'] + "--synth.txt", "w") as out:
    for s in range(len(synth)):
        # Print out star to file
        outstr = "%9.5f %9.5f " % (binary[s, 0], binary[s, 1])
        for i in range(17):
            outstr += "%6.3f " % synth.loc[s, f'{i*2}']
...
for m in range(len(minq_synth)):
    binqs = [synth_summary[x, 2] / synth_summary[x, 0] for x in range(synth_summary.shape[0]) if
             synth_summary[x, 0] > 0 and binary[x, 0] // minq_dm == m and binary[x, 1] == 0]
    if len(binqs) == 0:
        continue
...
for m in range(len(minq_mod)):
    if m * minq_dm > minmass:
        minq_mod[m] = minmass / (m * minq_dm + minq_dm / 2)

print("    Writing minimum mass ratio results to '%s--minq.txt'" % (options['data']))
with open(options['data'] + "--minq.txt", "w") as out:
    for m in range(len(minq_synth)):
        print("%5.2f  %5.3f  %5.3f  %5.3f" % (
        m * minq_dm, minq_synth[m], minq_mod[m], max([minq_synth[m], minq_mod[m], 0.3])), file=out)

#### UPDATED RESULTS
print("\nUpdating results...")
print("    Writing updated results to '%s--results.txt'" % (options['data']))
with open(options['data'] + "--results.txt", 'w') as out:
    for s in range(len(mag_df)):
        ...
        if summary[s, 0] == 0:
            ...
            bflag = summary[s, 8]
        elif summary[s, 2] < minmass or summary[s, 2] / summary[s, 0] < minq_synth[int(summary[s, 0] // minq_dm)] or \
                summary[s, 8] == 1:
        ...
        outstr = "%16s %9.5f %9.5f " % (info_df.loc[s, '2MASS ID'], info_df.loc[s, 'RA'], info_df.loc[s, 'DEC'])
        for i in range(17):
            outstr += "%6.3f " % mag_df.loc[s, f'{i*2}']
        outstr += "   %2d %2d %6.3f %6.4f %6.3f %6.4f" % (bflag, info_df.loc[s, 'RV Variability Index'], mpri, mprie, msec, msece)
        print(outstr, file=out)
