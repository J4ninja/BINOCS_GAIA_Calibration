...
def sedfit(singles, binary, mag, options, chicut=7.0, nvis=3, nnir=3, nmir=2, chi=False) -> np.ndarray:
    '''
    SUBROUTINE: SEDFIT
    ...
    INPUT:       singles -- isochrone data from READISO, MINTERP, or FIDISO
                 ...
                 mag -- star magnitude DataFrame from READDATA
                 ...
                 chi (optional, default = False) -- if True, use chi-squared kernel
    ...
    kernel_file = "%s/kernel/binocs_chi.c" % pwd if chi else "%s/kernel/binocs.c" % pwd
    with open(kernel_file, 'r') as df:
    ...
    # data, err = mag.iloc[:, ::2].copy(), mag.iloc[:, 1::2].copy()
    data = pd.DataFrame(index=mag.index)
    err = pd.DataFrame(index=mag.index)


    # Copy columns from mag_df to err if their titles contain 'err'
    for col_title in mag.columns:
        if 'err' in col_title:
            err[col_title] = mag[col_title]
        else:
            data[col_title] = mag[col_title]

    err.fillna(9.999, inplace=True)
    data.fillna(99.999, inplace=True)
    
    data[data < 80] -= options['m-M'] + options['ebv'] * 3.08642 * options['ak'][0]
    err[data >= 80] = 0

    # Flatten data and err for use in OpenCL kernel
    data, err = data.values.ravel(), err.values.ravel()
    ...
    if options['nruns'] < 200:
        p = 10
    elif options['nruns'] < 500:
        p = 30
    else:
        p = 50
    ...
    for r in range(options['nruns']):
        # Print progress
        if r < 1:
            print("    Run %3d of %3d " % (r, options['nruns']), end='')
...
def summarize(results, binary, singles):
    ...
    for s in range(results.shape[0]):
        ...
        summary.append([mpri, upri, msec, usec, medchi, smass, umass, smchi, bflag])

    columns = [
        "best-fit primary mass",
        "uncertainty on primary mass",
        "best-fit secondary mass",
        "uncertainty on secondary mass",
        "average \Sigma per filter",
        "best-fit single star model mass",
        "uncertainty in single star model mass",
        "average \Sigma per filter for single star model matches",
        "binary flag"
    ]
