        ...
        for o in range(len(optlines)):
            ...
            if len(id2mass) < 1:
                for l in datalines:
                    ...
                    self.extendarr(id2mass, mag, mempct, memchar, match)
                    ra[len(id2mass)-1] = thisra
                    dec[len(id2mass)-1] = thisdec
                    if is2mass == 1: id2mass[len(id2mass)-1] = tmp[0]
                    self.savemag(dataarr, mag, mempct, memchar, len(id2mass)-1, filters, maxerr)
            ...
            for i in range(len(indexes)):
                ...
                if (indexes[i] >= 0): self.savemag(dataarr, mag, mempct, memchar, indexes[i], filters, maxerr)
                else:
                    self.extendarr(id2mass, mag, mempct, memchar, match)
    ...
    def paysttrim(self, catalog):
        ...
        while choice != 0:
            ...
            if pmag < 0:
                ...
                pcolstr = input('What is the CMD color? ')
            ...
            if choice == 5:
                if isoplot == 0:
                    isonames = subprocess.check_output('ls ~/Documents/projects/isochrones/new/*.dat', shell=True).decode('utf-8').splitlines()
                    isoname = input('Enter isochrone name to overplot: ')
                    ...
                    removechoice = input('Adjust parameters? (y|n): ')
                    if removechoice == 'n':
                        isoplot = 0
